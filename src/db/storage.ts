  ...
  startNewCycle: async (nextYear: FinancialYear) => {
    ...
    Array.from(latestRecordsByEquipment.values()).forEach(r => {
      ...
      batch.set(doc(db, 'testRecords', newId), {
        ...
        testerName: '',
        testedBy: '',
        plantPersonnel: r.plantPersonnel || '',
        attachmentUrl: r.attachmentUrl || '',
        healthCondition: isUnsatisfactory ? 'Unsatisfactory' : 'Satisfactory',
        deficiency: r.deficiency || r.deficienciesText || '',
        remarks: r.remarks || r.overallSummaryText || '',
        complianceText: r.complianceText || '',
        overallSummaryText: r.overallSummaryText || '',
        actionTaken: '',
        updatedAt: new Date().toISOString(),
